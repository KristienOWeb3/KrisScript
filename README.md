...
- **The DM is a shop window, not a control panel.** Customers see the tiers a merchant offers only when the merchant updates their Subscriptions section in the SubScript dashboard by hand — no API publishes into a customer's DM, so **upgrading from the DM is not possible**. Checkouts are therefore not published either (`publishToDm: false`, sent as a literal — omitting it means published, since SubScript publishes unless it receives an explicit false). Catalogue plans created by `npm run plans:bootstrap` exist for two other reasons: a durable `planId` to subscribe against, and a `subscribeUrl` anyone can pay. Each carries a `detailsUrl` pointing at `/pricing`, so a merchant's dashboard entry can link back here — and the plan change happens on the platform, through a checkout we initiated and can therefore retire the old subscription from. `detailsUrl` is only sent when `APP_URL` is HTTPS, and price, period and that URL are immutable once published, so a plan bootstrapped from localhost needs replacing rather than repairing. **Note for mainnet:** a published plan carries no environment of its own and no plan query filters on one, so a test-key plan sits in the same public catalogue as live plans. Fine on testnet; re-check before mainnet.

### The four states the two platforms agree on

Subscribed, cancelled, resumed, gifted. Those are the only transitions a customer can cause that both sides have to reflect, and `lib/billing.ts` derives them once in `subscriptionStanding()` so `/pricing` and `/chat` cannot disagree — `/api/me` returns `standing` and `willRenew`, and neither page recomputes it.

| State | Arrives as | Effect here |
|---|---|---|
| Subscribed | `subscription.activated` | Grant the tier, expiry from the stated `currentPeriodEnd` |
| Cancelled | `subscription.cancel_scheduled` → `.canceled` | `canceling` then `canceled`; access runs to period end, plan not cleared |
| Resumed | `subscription.reactivated` | Extend to the newly stated period end — a resume **is** charged |
| Gifted | `payment.succeeded`, two different addresses | Credit the beneficiary, flag it as non-renewing |

Cancel and resume work from the customer's own SubScript DM; both arrive as ordinary webhooks. Upgrades do not, and cannot — they start on `/pricing`.

Handled internally but never surfaced as a state: `subscription.renewed` (this is what moves the period forward — miss it and a subscriber lapses while still being charged), `payment_failed`, and the `renewal_upcoming` / `trial_ending` / `allowance_low` advisories.

Building your own SubScript integration? [docs/INTEGRATING-SUBSCRIPT.md](docs/INTEGRATING-SUBSCRIPT.md) is what we wish we had read first.
...
lib/billing.ts             Idempotent fulfillment, subscription lifecycle, gifts, standing
...
docs/INTEGRATING-SUBSCRIPT.md What we learned wiring this up, for other merchants
```
