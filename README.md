...
- **The DM is a shop window, not a control panel.** Checkouts are *not* published (`publishToDm: false`, sent as a literal — omitting it means published, since SubScript publishes unless it receives an explicit false). A checkout is one customer starting one subscription, not a catalogue entry, and publishing each one put every attempt — abandoned ones included — into the DM plan picker as another switchable tier. Discovery instead comes from the catalogue plans created once by `npm run plans:bootstrap`, each carrying a `detailsUrl` that points back at `/pricing`. So the DM shows what the business offers and links out to it, and the plan change happens on the site, through a checkout we initiated and can therefore supersede the old subscription from. `detailsUrl` is only sent when `APP_URL` is HTTPS, and price and period are immutable once published — so a plan bootstrapped from localhost has a permanently missing link and needs replacing, not repairing. **Note for mainnet:** a published plan carries no environment of its own and no plan query filters on one, so a test-key plan is listed in the same public catalogue as live plans. Fine on testnet; re-check before mainnet.
